...
# Friends
# http://tieba.baidu.com/p/2444660096?see_lz=1&pn=1
# 只看楼主模式下的第一页
# nba迁移到Friends
# patternTitle
# patternCode都要改
...
class BDTB:
    ...
    def GetPageItems(self, pageCode):
        ...
        for item in findAllResults:            
            pageFloors.append(self.tools.replace(item).strip())
        return pageFloors
    def WriteInFile(self, pageFloors, page, title, pages, lzCount):
        f = codecs.open('Friends.txt','a+','utf-8')
        #f=open('tb.txt', 'a')
    ...
    def Start(self):
        ...
        patternTitle = re.compile('<h1.*?title=.*?>(.*?)</h1>', re.S)
        ...
        for page in range(pages):
            print u'现将第%s页写入文档Friends.txt' % (page+1)
...
crawler = BDTB('http://tieba.baidu.com/p/2444660096', 1)
crawler.Start()
